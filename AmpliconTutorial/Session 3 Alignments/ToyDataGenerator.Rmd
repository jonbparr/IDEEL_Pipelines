...
newPath = paste("/Users/nbrazeau/Desktop/test/", newPath,sep = "")
...
```{r Fasta generation, echo=F, warning=F, include=T}
set.seed(444)
...
```{bash Illumina Sequencer Simulation, echo=F, warning=F, include=T}
...
/usr/local/Cellar/art/20160605_1/bin/art_illumina -ss MSv3 -p -l 250 -f 500000 -m 600 -s 100 -i $OUTPUTDIRECTORY/MutmockPf.fa -o $OUTPUTDIRECTORY/MutmockPf
...
```{bash Running Flash, echo=F, warning=F, include=T}
...
```{bash Running gatk and bwa to prep fasta, echo=F, warning=F, include=T}
...
```{bash Running bwamem, echo=F, warning=F, include=T}
...
## Sort Bam
```{bash Running sortbam picard, echo=F, warning=F, include=T}
...
```{bash Running bcftools, echo=F, warning=F, include=T}
